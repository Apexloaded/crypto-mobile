...
export class DepositPage implements OnInit {
  ...
  public initDeposit() {
    ...
    if (
      this.selectedPlan &&
      this.depositForm.controls['amount'].value < this.selectedPlan.min_amount
    ) {
      this.utilServices.presentToast(
        `Your investment amount should not be less than ${this.selectedPlan.min_amount}`,
        3000
      );
      return;
    }
    ...
    if (
      this.selectedPlan &&
      this.depositForm.controls['amount'].value > this.selectedPlan.max_amount
    ) {
      this.utilServices.presentToast(
        `Your investment amount should not be more than ${this.selectedPlan.max_amount}, kindly choose a higher plan!`,
        3000
      );
      return;
    }
    ...
    this.utilServices.presentLoading('circles').then((el) => {
      this.transactionProv
        ...
        .then((res) => {
          ...
          this.utilServices.presentModal(modalProps);
        })
        .catch((error) => {
          this.utilServices.dismisLoadCtrl();
